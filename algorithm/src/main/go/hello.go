...
/*
	除了直接 import，常见的 import 有三种特殊形式（或者叫前缀）
	分别是 前面加下划线(_)、加点(.)、加别名

	一、加下划线(_)
	import 下划线（如：_ "github.com/go-sql-driver/mysql"）的作用：当导入一个包时，该包下的文件里所有 init() 函数都会被执行
	有些时候我们并不需要把整个包都导入进来，仅仅是希望它执行 init() 函数而已。这个时候就可以使用 import _ 引用的包名，即：使用 [import _ 包路径]
	引用该包，仅仅是为了调用 init() 函数，所以是无法通过包名来调用包中的其他函数
...
/*
	init函数会在所有程序执行开始前被调用，每个包可以包含多个init函数，所有被编辑器识别到的init函数都会在main函数执行前被调用。
	通常被用来注册一个程序需要使用的依赖，如mysql注册，配置文件加载等

	每个包中可以有多个init函数，而其他包的init是在main包的init调用之前被执行，main函数最后执行即可，如果非要有依赖的顺序关系，那么可以在init中包含调用即可
	...
	init函数不可被调用，调用会提示：undefined: init
...
func main() {
	/*
	全局变量 和 局部变量 同名问题，Java中可以使用 this.xxx 指定选用全局变量
	但是 因为 go 不支持重载，所以 this 没啥用。this 这个玩意最重要的地方是在构造函数和继承多态，重载。主要还是在于构造函数、继承多态的区别上
	*/
